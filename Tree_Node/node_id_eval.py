...
def worker(eval_tree, eval_target, depth, string_target, string_tree, lengths, spell, temp_folder):
    ...
    # Brotli
    os.system(f"brotli -f -q 11 {temp_folder}/*_{depth}_{string_target}_{string_tree}.txt")
    os.system(f"brotli -f -q 11 {temp_folder}/*_{depth}_{string_target}_{string_tree}.bin")
    os.system(f"brotli -f -q 11 {temp_folder}/*_{depth}_{string_target}_{string_tree}.bin.rand")
...
# Brotli
    for length in lengths:
        res[f"{length}"]["TxBRT"]=Path(f"{temp_folder}/target{length}_{depth}_{string_target}_{string_tree}.txt.br").stat().st_size

    for length in lengths:
        res[f"{length}"]["OppBRT"]=Path(f"{temp_folder}/target{length}_{depth}_{string_target}_{string_tree}.bin.br").stat().st_size

    for length in lengths:
        res[f"{length}"]["OppRandBRT"]=Path(f"{temp_folder}/target{length}_{depth}_{string_target}_{string_tree}.bin.rand.br").stat().st_size
    ...
    # Brotli
    df["BRTRatio"] = df["TxBRT"]/df["Txt"]
    df["OppBRTRatio"] = df["OppBRT"]/df["Txt"]
    df["OppRandBRTRatio"] = df["OppRandBRT"]/df["Txt"]

    return {
        "tree":string_tree,
        "target":string_target,
        "OppRatio":dict(df["OppRatio"]),
        "GzRatio":dict(df["GzRatio"]),
        "OppGzRatio":dict(df["OppGzRatio"]),
        "OppRandGzRatio":dict(df["OppRandGzRatio"]),
        "BRTRatio": dict(df["BRTRatio"]),
        "OppBRTRatio": dict(df["OppBRTRatio"]),
        "OppRandBRTRatio": dict(df["OppRandBRTRatio"]),
    }
